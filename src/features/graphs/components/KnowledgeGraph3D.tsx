...
const COLORS = {
  ...
  subtopic: "#818cf8", // Light purple - matches hub theme
};
...
export default function KnowledgeGraph2D({ data }: KnowledgeGraph2DProps) {
  ...
  const graphData: GraphData = useMemo(() => {
    ...
    const links: GraphLink[] = (data.edges || [])
      ...
      .map((edge) => ({
        ...
        type: edge.type,
        color: edge.type === "IS_PREREQUISITE_FOR" ? COLORS.prerequisite : COLORS.subtopic,
      }));
  ...
  useEffect(() => {
    if (graphRef.current) {
      ...
      // 松散图谱 (density < 1.5): 减少斥力，节点更紧凑
      // 紧密图谱 (density >= 1.5): 增加斥力，避免重叠
  ...
  return (
    <div className="relative w-full h-full" style={{ background: COLORS.bg }}>
      ...
      >
        <h2 className="text-white text-lg font-semibold mb-3">Knowledge Graph</h2>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <div className="flex items-center text-xs text-gray-400">
            <div
              className="w-4 h-0.5 mr-1"
              style={{ background: COLORS.subtopic }}
            />
            Subtopic
          </div>
        </div>
      ...
      <ForceGraph2D
        ...
        nodeLabel={(node: GraphNode) => `
          ...
          ">
            ...
            <div style="font-size: 11px; color: #888; line-height: 1.5;">${node.description}</div>
